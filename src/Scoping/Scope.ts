export module Scope {
    ...
    export function empty<T>(): Scope<T> {
        return Scope.ofNullable<T>(null)
    }
    ...
    export function ofElse<U>(v: U | null | undefined, elseValue: U): Scope<U> {
        return Scope.ofNullable(v).ifEmpty(elseValue);
    }
    ...
    export class ScopeImpl<T> implements Scope<T> {
        ...
        ifNonEmptyMap<O>(op: (t: T) => (O | null)): Scope.Scope<O> {
            if (this.isEmpty()) {
                return Scope.empty<O>()
            } else {
                return Scope.ofNullable(op(this.get()))
            }
        }

        ifThenApply(test: (t: T) => boolean, apply: (t: T) => void): Scope.Scope<T> {
        ...
        ifThenMap<O>(cond: (t: T) => boolean, mapTo: (t: T) => (O | null), elseOperation: () => O | null): Scope.Scope<O> {
            if (this.isEmpty()) {
                return Scope.empty<O>()
            } else if (cond(this.get())) {
                return Scope.ofNullable(mapTo(this.get()))
            } else {
                return Scope.ofNullable(elseOperation())
            }
        }
